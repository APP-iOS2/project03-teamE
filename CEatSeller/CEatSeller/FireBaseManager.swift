...
final class CEatsFBManager {
    ...
    func addRTROrderSnapshot(data: Restaurant, completion: @escaping (Order) -> ()) {
        let collectionRef = db.collection("Order")
        ...
        DispatchQueue.global().async {
            listener = collectionRef.addSnapshotListener { snapshot, error in
                ...
                if !myChange.isEmpty {
                    guard let result = try? myChange[0].data(as: Order.self) else {
                        print(#function + ": fail to optional bind")
                        return
                    }
                    completion(result)
